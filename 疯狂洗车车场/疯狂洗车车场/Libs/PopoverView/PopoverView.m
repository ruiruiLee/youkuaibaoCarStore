           ...
           withText:(NSString *)text
{
    ...
    CGSize textSize = [text
                       sizeWithFont:font
                       constrainedToSize:CGSizeMake(screenSize.width - kHorizontalMargin * 4.f,
                                                    1000.f)
    ...
    [textView setNumberOfLines:0]; // This is so the label word wraps instead of
    ...
    [self showAtPoint:point
               inView:view
        withViewArray:[NSArray arrayWithObject:[textView AUTORELEASE]]];
}
      ...
      withViewArray:(NSArray *)viewArray
{
    ...
    // Position each view the first time, and identify which view has the largest
    // width that controls
    // the sizing of the popover.
    for (UIView *view in viewArray)
    {
        
        view.frame = CGRectMake(0, totalHeight, view.frame.size.width,
    ...
    // If dividers are enabled, then we allocate the divider rect array.  This
    // will hold NSValues
    if (kShowDividersBetweenViews) {
        dividerRects =
        [[NSMutableArray alloc] initWithCapacity:viewArray.count - 1];
    }
    ...
    // Now we actually change the frame element for each subview, and center the
    // views horizontally.
    for (UIView *view in viewArray) {
        if ([view autoresizingMask] == UIViewAutoresizingFlexibleWidth) {
            ...
            view.frame = CGRectMake(view.frame.origin.x, view.frame.origin.y,
                                    totalWidth, view.frame.size.height);
        } else {
            // If the view is not flexible width, then we position it centered in the
            // view
            // without stretching it.
            view.frame =
            CGRectMake(floorf(CGRectGetMinX(boxFrame) + totalWidth * 0.5f -
                              view.frame.size.width * 0.5f),
                       view.frame.origin.y, view.frame.size.width,
                       view.frame.size.height);
        }
        
        // and if dividers are enabled, we record their position for the drawing
        // methods
        if (kShowDividersBetweenViews && i != viewArray.count - 1) {
            CGRect dividerRect =
            CGRectMake(view.frame.origin.x,
                       floorf(view.frame.origin.y + view.frame.size.height +
                              kBoxPadding * 0.5f),
                       view.frame.size.width, 0.5f);
            
            [((NSMutableArray *)dividerRects)
             addObject:[NSValue valueWithCGRect:dividerRect]];
        }
      ...
      withViewArray:(NSArray *)viewArray
{
    ...
    UILabel *titleLabel = [[UILabel alloc]
                           initWithFrame:CGRectMake(0.f, 0.f, titleSize.width, titleSize.height)];
    ...
    // Make sure that the title's label will have non-zero height.  If it has zero
    // height, then we don't allocate any space
    ...
    titleLabel.frame =
    CGRectMake(floorf(totalWidth * 0.5f - titleSize.width * 0.5f), 0,
               titleSize.width, titleSize.height);
    
    // Store the titleView as an instance variable if it is larger than 0 height
    // (not an empty string)
    ...
    withStringArray:(NSArray *)stringArray
{
    ...
    [[NSMutableArray alloc] initWithCapacity:stringArray.count];
    
    UIFont *font = kTextFont;
    
    for (NSString *string in stringArray) {
        CGSize textSize = [string sizeWithFont:font];
        UIButton *textButton = [[UIButton alloc]
                                initWithFrame:CGRectMake(0, 0, textSize.width, textSize.height)];
        ...
        [textButton setTitleColor:kTextHighlightColor
    ...
    withStringArray:(NSArray *)stringArray
{
    ...
    [self showAtPoint:point
               inView:view
            withTitle:title
        withViewArray:[labelArray AUTORELEASE]];
}
     ...
     withImageArray:(NSArray *)imageArray
{
    ...
    // We create an array of subviews that contains the strings and images
    // centered above a label.
    
    NSAssert((stringArray.count == imageArray.count),
             @"stringArray.count should equal imageArray.count");
    NSMutableArray *tempViewArray =
    [self makeTempViewsWithStrings:stringArray andImages:imageArray];
     ...
     withImageArray:(NSArray *)imageArray
{
    ...
    [self showAtPoint:point
               inView:view
            withTitle:title
        withViewArray:tempViewArray];
}

- (NSMutableArray *)makeTempViewsWithStrings:(NSArray *)stringArray
                                   andImages:(NSArray *)imageArray
{
    ...
    for (int i = 0; i < stringArray.count; i++) {
        ...
        float containerWidth = imageView.frame.size.width + label.frame.size.width;
        NSLog(@"%f", imageView.frame.size.width);
        NSLog(@"%f", imageView.frame.size.height);

        float containerHeight =  MAX(label.frame.size.height + kImageTopPadding +
                                     kImageBottomPadding, kImageTopPadding +
                                     kImageBottomPadding + imageView.frame.size.height);
        ...
        // Now we do the frame manipulations to put the imageView on top of the
        // label, both centered
        ...
        label.frame = CGRectMake(2 + imageView.frame.size.height,
                                 kImageTopPadding,
    ...
    withContentView:(UIView *)cView
{
    [self showAtPoint:point
               inView:view
            withTitle:title
        withViewArray:[NSArray arrayWithObject:cView]];
}
    ...
    withContentView:(UIView *)cView
{
    ...
    topView = /*[[*/ [[[UIApplication
                        sharedApplication] delegate] window] /*subviews] lastObject]*/;
    ...
    // First stage animates to 1.05x normal size, then second stage animates back
    // down to 1x size.
...
- (void)setupLayout:(CGPoint)point inView:(UIView *)view
{
    ...
    if (arrowPoint.x + kArrowHeight >
        topViewBounds.size.width - kHorizontalMargin - kBoxRadius -
        kArrowHorizontalPadding) { // Too far to the right
        arrowPoint.x = topViewBounds.size.width - kHorizontalMargin - kBoxRadius -
        kArrowHorizontalPadding - kArrowHeight;
        // NSLog(@"Correcting Arrow Point because it's too far to the right");
    } else if (arrowPoint.x - kArrowHeight <
               kHorizontalMargin + kBoxRadius +
               kArrowHorizontalPadding) { // Too far to the left
        arrowPoint.x =
        kHorizontalMargin + kArrowHeight + kBoxRadius + kArrowHorizontalPadding;
    ...
    // Check to see if the centered xOrigin value puts the box outside of the
    // normal range.
    if (xOrigin < CGRectGetMinX(topViewBounds) + kHorizontalMargin) {
        xOrigin = CGRectGetMinX(topViewBounds) + kHorizontalMargin;
    } else if (xOrigin + boxWidth >
               CGRectGetMaxX(topViewBounds) - kHorizontalMargin) {
        // Check to see if the positioning puts the box out of the window towards
        // the left
    ...
    if (topPoint.y - contentHeight - arrowHeight - topPadding <
        CGRectGetMinY(topViewBounds)) {
        // Position below because it won't fit above.
        above = NO;
        
        boxFrame =
        CGRectMake(xOrigin, arrowPoint.y + arrowHeight, boxWidth, boxHeight);
    } else {
        // Position above.
        above = YES;
        
        boxFrame = CGRectMake(xOrigin, arrowPoint.y - arrowHeight - boxHeight,
                              boxWidth, boxHeight);
    }
    
    // NSLog(@"boxFrame:(%f,%f,%f,%f)", boxFrame.origin.x, boxFrame.origin.y,
    // boxFrame.size.width, boxFrame.size.height);
    
    CGRect contentFrame =
    CGRectMake(boxFrame.origin.x + padding, boxFrame.origin.y + padding,
               contentWidth, contentHeight);
    contentView.frame = contentFrame;
    
    // We set the anchorPoint here so the popover will "grow" out of the
    // arrowPoint specified by the user.
    // You have to set the anchorPoint before setting the frame, because the
    // anchorPoint property will
    // implicitly set the frame for the view, which we do not want.
    self.layer.anchorPoint =
    CGPointMake(arrowPoint.x / topViewBounds.size.width,
                arrowPoint.y / topViewBounds.size.height);
    ...
    // Add a tap gesture recognizer to the large invisible view (self), which will
    // detect taps anywhere on the screen.
    UITapGestureRecognizer *tap =
    [[UITapGestureRecognizer alloc] initWithTarget:self
                                            action:@selector(tapped:)];
    tap.cancelsTouchesInView = NO; // Allow touches through to a UITableView or
    // other touchable view, as suggested by
    // Dimajp.
...
- (void)showActivityIndicatorWithMessage:(NSString *)msg
{
    ...
    CGRectMake(CGRectGetMidX(contentView.bounds) - 10.f,
               CGRectGetMidY(contentView.bounds) - 10.f + 20.f, 20.f, 20.f);
...
- (void)hideActivityIndicatorWithMessage:(NSString *)msg
{
    ...
    [UIView animateWithDuration:0.1f
                     animations:^{ activityIndicator.alpha = 0.f; }
                     completion:^(BOOL finished) {
                         [activityIndicator RELEASE];
                         [activityIndicator removeFromSuperview];
                         activityIndicator = nil;
...
- (void)showImage:(UIImage *)image withMessage:(NSString *)msg
{
    ...
    imageView.frame = CGRectMake(
                                 floorf(CGRectGetMidX(contentView.bounds) - image.size.width * 0.5f),
                                 floorf(CGRectGetMidY(contentView.bounds) - image.size.height * 0.5f +
                                        ((self.titleView) ? 20 : 0.f)),
...
- (void)showError
{
    ...
    [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"error"]];
    imageView.alpha = 0.f;
    imageView.frame = CGRectMake(CGRectGetMidX(contentView.bounds) - 20.f,
                                 CGRectGetMidY(contentView.bounds) - 20.f +
                                 ((self.titleView) ? 20 : 0.f),
                                 40.f, 40.f);
    imageView.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
...
{
    UIImageView *imageView =
    [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"success"]];
    imageView.alpha = 0.f;
    imageView.frame = CGRectMake(CGRectGetMidX(contentView.bounds) - 20.f,
                                 CGRectGetMidY(contentView.bounds) - 20.f +
                                 ((self.titleView) ? 20 : 0.f),
...
- (void)tapped:(UITapGestureRecognizer *)tap
{
    ...
    for (int i = 0; i < subviewsArray.count && !found; i++) {
        ...
        // NSLog(@"Rect:(%f,%f,%f,%f)", view.frame.origin.x, view.frame.origin.y,
        // view.frame.size.width, view.frame.size.height);
        
        if (CGRectContainsPoint(view.frame, point)) {
            // The tap was within this view, so we notify the delegate, and break the
            // loop.
            ...
            //
            if (delegate &&
                [delegate respondsToSelector:@selector(popoverView:
...
- (void)dismiss:(BOOL)animated
{
    if (!animated) {
        ...
        [UIView animateWithDuration:0.3f
                         ...
                         completion:^(BOOL finished) { [self dismissComplete]; }];
    }
}
...
- (void)dismissComplete
{
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(popoverViewDidDismiss:)]) {
        [delegate popoverViewDidDismiss:self];
    }
}

- (void)animateRotationToNewPoint:(CGPoint)point
                           inView:(UIView *)view
...
- (void)drawRect:(CGRect)rect
{
    ...
    float cpOffset = kCPOffset; // Control Point Offset.  Modifies how "curved"
    // the corners are.
    ...
    [popoverPath moveToPoint:CGPointMake(CGRectGetMinX(frame),
                                         CGRectGetMinY(frame) + radius)]; // LT1
    [popoverPath
     addCurveToPoint:CGPointMake(xMin + radius, yMin)
     controlPoint1:CGPointMake(xMin, yMin + radius - cpOffset)
     controlPoint2:CGPointMake(xMin + radius - cpOffset, yMin)]; // LT2
    
    // If the popover is positioned below (!above) the arrowPoint, then we know
    // that the arrow must be on the top of the popover.
    // In this case, the arrow is located between LT2 and RT1
    if (!above) {
        [popoverPath addLineToPoint:CGPointMake(arrowPoint.x - kArrowHeight,
                                                yMin)]; // left side
        [popoverPath addCurveToPoint:arrowPoint
                       controlPoint1:CGPointMake(arrowPoint.x - kArrowHeight +
                                                 kArrowCurvature,
                                                 yMin)
        ...
        [popoverPath addCurveToPoint:CGPointMake(arrowPoint.x + kArrowHeight, yMin)
                       ...
                       controlPoint2:CGPointMake(arrowPoint.x + kArrowHeight -
                                                 kArrowCurvature,
                                                 yMin)]; // right side
    }
    ...
    [popoverPath
     addCurveToPoint:CGPointMake(xMax, yMin + radius)
    ...
    [popoverPath
     addCurveToPoint:CGPointMake(xMax - radius, yMax)
     controlPoint1:CGPointMake(xMax, yMax - radius + cpOffset)
     controlPoint2:CGPointMake(xMax - radius + cpOffset, yMax)]; // RB2
    
    // If the popover is positioned above the arrowPoint, then we know that the
    // arrow must be on the bottom of the popover.
    ...
    [popoverPath
     addCurveToPoint:CGPointMake(xMin, yMax - radius)
    ...
    [NSArray arrayWithObjects:(id)kGradientTopColor.CGColor,
     (id)kGradientBottomColor.CGColor, nil];
    CGFloat gradientLocations[] = {0, 1};
    CGGradientRef gradient =
    CGGradientCreateWithColors(colorSpace,
                               (CFTYPECAST(CFArrayRef) gradientColors),
                               gradientLocations);
    
    // These floats are the top and bottom offsets for the gradient drawing so the
    // drawing includes the arrows.
    ...
    CGContextDrawLinearGradient(context, gradient,
                                CGPointMake(CGRectGetMidX(frame),
                                            CGRectGetMinY(frame) - topOffset),
                                CGPointMake(CGRectGetMidX(frame),
                                            CGRectGetMaxY(frame) + bottomOffset),
                                0);
    ...
    if (kDrawTitleGradient) {
        ...
        if (titleBGHeight > 0.f) {
            ...
            [titleBGPath
             addLineToPoint:CGPointMake(CGRectGetMinX(frame),
                                        CGRectGetMinY(frame) + radius)]; // LT1
            [titleBGPath
             addCurveToPoint:CGPointMake(xMin + radius, yMin)
             controlPoint1:CGPointMake(xMin, yMin + radius - cpOffset)
            ...
            // If the popover is positioned below (!above) the arrowPoint, then we
            // know that the arrow must be on the top of the popover.
            // In this case, the arrow is located between LT2 and RT1
            if (!above) {
                [titleBGPath addLineToPoint:CGPointMake(arrowPoint.x - kArrowHeight,
                                                        yMin)]; // left side
                [titleBGPath addCurveToPoint:arrowPoint
                               ...
                               controlPoint2:arrowPoint]; // actual arrow point
                [titleBGPath
                 addCurveToPoint:CGPointMake(arrowPoint.x + kArrowHeight, yMin)
                 controlPoint1:arrowPoint
                 controlPoint2:CGPointMake(arrowPoint.x + kArrowHeight -
                                           kArrowCurvature,
                                           yMin)]; // right side
            ...
            //// Gradient Declarations
            NSArray *gradientColors =
            [NSArray arrayWithObjects:(id)kGradientTitleTopColor.CGColor,
            ...
            // These floats are the top and bottom offsets for the gradient drawing so
            // the drawing includes the arrows.
            float topOffset = (!above ? kArrowHeight : 0.f);
            ...
            CGContextDrawLinearGradient(
                                        context, gradient,
                                        CGPointMake(CGRectGetMidX(frame), CGRectGetMinY(frame) - topOffset),
                                        CGPointMake(CGRectGetMidX(frame),
                                                    CGRectGetMinY(frame) + titleBGHeight),
                                        0);
            CGContextEndTransparencyLayer(context);
            CGContextRestoreGState(context);
            
            UIBezierPath *dividerLine = [UIBezierPath
                                         bezierPathWithRect:CGRectMake(startingPoint.x, startingPoint.y,
    ...
    // The border is done last because it needs to be drawn on top of everything
    // else
    if (kDrawBorder) {
