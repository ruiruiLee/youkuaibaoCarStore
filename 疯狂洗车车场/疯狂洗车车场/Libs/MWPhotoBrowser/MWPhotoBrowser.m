...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self)
    {
        ...
        [backBtn setBackgroundImage:[UIImage imageNamed:@"back_btn"]
                           forState:UIControlStateNormal];
        [backBtn setFrame:CGRectMake(0, 12, 10, 16)];
        [backBtn addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        

        // We're first on stack so show done button
        _doneButton = [[UIBarButtonItem alloc] initWithCustomView:backBtn];
        // Set appearance
...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    if ([[UINavigationBar class] respondsToSelector:@selector(appearance)]) {
        [navBar setBackgroundImage:[UIImage imageNamed:@"navi_bg"] forBarMetrics:UIBarMetricsDefault];
        [navBar setBackgroundImage:[UIImage imageNamed:@"navi_bg"] forBarMetrics:UIBarMetricsLandscapePhone];
    }
}
...
- (void)updateNavigation {
    ...
    } else if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)])
    {
        if (numberOfPhotos > 1)
        {
            self.title = [NSString stringWithFormat:@"%@%lu %@ %lu",
                          [_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex], (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"/", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
        else
        {
            self.title = [NSString stringWithFormat:@"%@1/1", [_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex]];
        }
	}
    else
    {
